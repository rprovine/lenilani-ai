/**
 * Populate production database with sample leads
 * Run this against: https://ai-bot-special.lenilani.com
 */
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.leadScore !== undefined) {
          console.log(`     📊 Score: ${data.leadScore}/100`);
        }
...
(async () => {
  console.log(`🔍 Checking production server: ${PRODUCTION_URL}`);

  if (!await checkServer()) {
    console.error(`\n❌ Server not responding at ${PRODUCTION_URL}`);
    process.exit(1);
  }

  console.log('✅ Server running!\n');
  console.log('📊 Populating production with sample leads...');
